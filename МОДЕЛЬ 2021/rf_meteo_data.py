...
def get_center_values(x, min_q, max_q):
    if x > min_q and x < max_q:
        return True
    else:
        return False
...
file_path = './output/METEO_table_CFO.xlsx'
writer = pd.ExcelWriter(file_path)

# Создание файла с ошибками
err_file = open('./output/meteo_err.txt', 'w')
...
#     precip_temp = precip_temp.loc[precip_temp_years_list]
    ...
    X_data_raw_0 = agro_data[['Янв_R',
                        ...
                        'Mean_Jun-Jul_T',
                        'Sum_Mar_Aug_T', 
						'ГТК']] # Предварительный список

    X_data_raw = X_data_raw_0[['Янв_R',
                        'Фев_R', 
                        'Мар_R', 
                        'Апр_R', 
                        'Май_R', 
    ...
    crops_list = ['Зерновые т/га',
                ...
                'Масличные 1 т/га', 
                'Зернобоб 1 т/га',
                'Зерн и зернобоб 1 т/га', 'Картофель т/га']
    ...
    # Расчитываем среднее по метео данным
    area_agriculture = X_data_raw[['Янв_R',
                                 'Мар_R', 
                                 'Май_R', 
                                 'Июн_R', 
                                 'Июл_R', 
    ...
    for crop in crops_list:
        ...
        X_data = current_crop_data[[ 'Апр_R', 
                                     'Май_R', 
                                     'Июн_R', 
                                     'Year_Sum_R', 
                                     'Sum_Mar-May_R', 
        ...
        while all_coefs_valid == False:
            ...
            if beta_pv_df['P_value'].empty:
                max_pv = False
            else:
                max_pv = max(beta_pv_df['P_value'])
            ...
            for row_number, row_name in enumerate(beta_pv_df.index):
                ...
                if len(beta_pv_df.index) == 1:
                    ...
                    continue
                elif current_p_value > 0.05 and current_p_value == max_pv and max_pv:
        ...
        if crop == 'Зерновые т/га':
            ...
            quantile_min = cereals_potencials.quantile(0.05)
            quantile_max = cereals_potencials.quantile(0.95)
            cereals_potencials = pd.Series(list((item for item in cereals_potencials.values.tolist() if get_center_values(item, quantile_min, quantile_max))))
            quant_value = cereals_potencials.mean()
...
#         cereals_potencials.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=145, index=False, header=False)
        agro_data.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=284)
        agro_data_corr = X_data_raw.join(agro_data[crops_list]).dropna(axis=1, thresh=10).dropna(axis=0)
        agro_data_corr.to_csv('./source_data/meteo_data_corr.csv')
        agc = pd.read_csv('./source_data/meteo_data_corr.csv', index_col=0)
        agro_data_corr = agc.corr()
        agro_data_corr.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=284 + agro_data.shape[0] + 2)
...
pivot.to_excel('./output/meteo_pivot.xlsx')
