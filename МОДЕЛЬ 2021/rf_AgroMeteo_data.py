...
areas = data_area['Территория'].unique()[2:3] # Нужно указывать районы цифрами
...
file_path = './output/AGRO_METEO_table_CFO.xlsx'
...
for area in areas:
    ...
    area_pivot['R при E > 10'] = precipitation_sum
    area_pivot['ГТК'] = gtk
...
#         cereals_potencials.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=145, index=False, header=False)
        agro_data.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=235)

        agro_data_corr = X_data_raw.join(agro_data[crops_list]).dropna(axis=1, thresh=10).dropna(axis=0)
        agro_data_corr.to_csv('./source_data/agro_meteo_data_corr.csv')
        agc = pd.read_csv('./source_data/agro_meteo_data_corr.csv', index_col=0)
        agro_data_corr = agc.corr()
        agro_data_corr.to_excel(writer, sheet_name=sheet_name, startcol=0, startrow=235 + agro_data.shape[0] + 2)
...
pivot.to_excel('./output/agro_meteo_pivot.xlsx')
