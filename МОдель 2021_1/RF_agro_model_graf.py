...
def make_area_main_table(area, data_area, data_sbor, data_ferts, data_power, data_techs, data_yield):
    ...
    #вычисляем зерновые
    zern_area = pd.concat((zern_zernbob_area.iloc[:,0],
                        zern_zernbob_area.iloc[:,4],
                        zern_bob_area.iloc[:,4]), axis=1)
    zern_area.columns = ['Территория', 'Зерн и зернобобовые 1000 га', 'Зернобобовые 1000 га']
    zern_area['Зерновые 1000 га'] = zern_area.iloc[:,1]-zern_area.iloc[:,2]
    zern_area['Зерновые доля'] = zern_area.iloc[:,2]/zern_area.iloc[:,1]
    
    zern_prod =  pd.concat((zern_bob_prod.iloc[:,0],
                        zern_zernbob_prod.iloc[:,4],
                        zern_bob_prod.iloc[:,4]), axis=1)
    zern_prod.columns = ['Территория', 'Зерн и зернобобовые 1000 т', 'Зернобобовые 1000 т']
    zern_prod['Зерновые 1000 т'] = zern_prod.iloc[:,1]-zern_prod.iloc[:,2]
    
    agro_data = pd.concat((zern_area, zern_prod.iloc[:,1:4]), axis=1)
    agro_data['Зерновые т/га'] = agro_data.iloc[:,7]/agro_data.iloc[:,3]
    agro_data['Зерн и зернобоб т/га'] = zern_zernbob_yield
    agro_data['Зернобоб т/га'] = zern_bob_yield
    ...
    potat_yield = potat_prod['Валовый сбор']/potat_area['Посевные площади'] 
    ...
    agro_data ['Овощи 1000 т'] = veget_otkr_zakr_prod.iloc[:,4]
    ...
    agro_data['МУ ВСЕГО'] = ferts_all.iloc[:,3]
    ...
    agro_data['МУ Овощи'] = ferts_veget.iloc[:,3]
    ...
    agro_data['МУ СХ Культуры Всего'] = ferts_sh_all.iloc[:,3]
...
def main():
    ...
    #УРОЖАЙНОСТИ https://fedstat.ru/indicator/31533 Этот файл используется, если нельзя получить информацию из предыдущих файлов
    ...
    # Get areas names
    areas_CFO = data_area['Территория'].unique()[0:19] #int(sys.argv[1])
    ...
    for area in areas_CFO:
        ...
        X_data_raw = crops_land_use[['МУ ВСЕГО', 
