...
Areas_CFO = data_area['Территория'].unique()[18] #ЧИСЛО РЕГИОНОВ
...
#					potenc.drop([0], inplace=True)
			...
			statistic_values_result.to_excel(writer, startcol=1, startrow=len(X_data.columns)+ 7 + startrow, index=False, header=False, sheet_name=sheet_name)
...
#			potenc.to_excel(writer, sheet_name=sheet_name, startcol=96, startrow=62)
		...
		startrow = startrow + len(X_data.columns)+12
